...
export default function CompanyDetailScreen() {
  ...
  const heroImage = company.photos?.[0]?.url || "https://images.unsplash.com/photo-1524504388940-b1c1722653e1?auto=format&fit=crop&w=1200&q=80";

  // Helper to get day name sk
  const days = { monday: 'Pondelok', tuesday: 'Utorok', wednesday: 'Streda', thursday: 'Štvrtok', friday: 'Piatok', saturday: 'Sobota', sunday: 'Nedeľa' };
    ...
    <ScrollView className="flex-1 bg-background">
      ...
      <View className="p-5 -mt-6 bg-background rounded-t-3xl shadow-sm">
        ...
        <View className="flex-row flex-wrap gap-2 mb-6">
            <View className="bg-gray-100 px-2 py-1 rounded"><Text className="text-xs text-gray-600">Bezplatné Wi-Fi</Text></View>
            <View className="bg-gray-100 px-2 py-1 rounded"><Text className="text-xs text-gray-600">Bezplatné parkovanie</Text></View>
            <View className="bg-gray-100 px-2 py-1 rounded"><Text className="text-xs text-gray-600">Platba kartou</Text></View>
        </View>
        ...
        <View className="space-y-4 mb-8">
            {services.map((service) => (
                <View key={service.id} className="flex-row justify-between items-center p-4 bg-white rounded-xl border border-gray-100 shadow-sm">
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                    {company.photos.slice(1).map((photo: any, index: number) => (
                        <Image 
                            ...
                            className="w-40 h-40 rounded-xl mr-3 bg-gray-200"
                        />
                    ))}
                </ScrollView>
            </View>
            ...
            <Text className="text-xl font-bold text-text-main mb-4">Hodnotenia zákazníkov</Text>
             <View className="flex-row items-baseline mb-4">
                ...
                <View className="flex-row">
                    {[1, 2, 3, 4, 5].map((star) => (
                        <FontAwesome key={star} name={star <= Math.round(Number(averageRating)) ? "star" : "star-o"} size={16} color="#d4a373" />
                    ))}
                </View>
                    ...
                    return (
                        <View key={rating.id} className="mb-4 border-b border-gray-100 pb-4">
                            ...
                            <View className="flex-row mb-2">
                                {[1, 2, 3, 4, 5].map((star) => (
                                    <FontAwesome key={star} name={star <= rating.rating ? "star" : "star-o"} size={12} color="#ffc107" />
                                ))}
                            </View>
        ...
        <View className="mb-8">
             ...
             {sortedHours?.map((hours: any) => (
                 <View key={hours.id} className="flex-row justify-between mb-1">
                     <Text className="text-text-muted capitalize">{(days as any)[hours.day_in_week] || hours.day_in_week}</Text>
                     <Text className="text-text-main">{hours.from_time ? `${hours.from_time.slice(0, 5)} - ${hours.to_time.slice(0, 5)}` : 'Zatvorené'}</Text>
                 </View>
             ))}
